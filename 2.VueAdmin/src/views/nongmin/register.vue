	...
	<div>
		<div class="register_view">
			<el-form :model="registerForm" class="register_form">
				...
				<div class="list_item">
					<div class="list_label">农民密码：</div>
					<el-input class="list_inp"
						 v-model="registerForm.nongminmima" 
						 placeholder="请输入农民密码"
						 type="password"
						 />
				</div>
				<div class="list_item">
					<div class="list_label">确认农民密码：</div>
					<el-input class="list_inp" v-model="registerForm.nongminmima2" type="password" placeholder="请输入确认农民密码" />
				</div>
				<div class="list_item">
...
<script setup>
	...
	const handleRegister = () => {
		...
		if((!registerForm.value.nongminzhanghao)){
			context?.$toolUtil.message(`农民账号不能为空`,'error')
			return false
		}
		if((!registerForm.value.nongminmima)){
			context?.$toolUtil.message(`农民密码不能为空`,'error')
			return false
		}
		if(registerForm.value.nongminmima!=registerForm.value.nongminmima2){
			context?.$toolUtil.message('两次密码输入不一致','error')
			return false
		}
		if((!registerForm.value.nongminxingming)){
...
<style lang="scss" scoped>
	...
	.register_view {
		...
		.register_form {
			...
			margin: 0 auto;
			flex-direction: column;
			background: url(http://codegen.caihongy.cn/20230811/35998be3b95844a68cab0b7410497ea1.png) no-repeat center top / 100% 100%;
			display: flex;
			width: 688px;
			min-height: 800px;
			justify-content: flex-start;
		...
		.list_btn {
			...
			.register {
					...
					margin: 0 auto;
					color: #fff;
					background: linear-gradient(180deg, rgba(163,207,63,1) 0%, rgba(99,126,36,1) 100%);
					width: auto;
					font-size: 16px;
					min-width: 120px;
					height: 40px;
