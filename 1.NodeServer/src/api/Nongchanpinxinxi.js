...
import NongchanpinxinxiModel from '../models/NongchanpinxinxiModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let where = {}
			let chanpinmingcheng = req.query.chanpinmingcheng
			if (chanpinmingcheng) {

				if (chanpinmingcheng.indexOf('%') != -1) {
					where.chanpinmingcheng = {
						[Op.like]: chanpinmingcheng
					}
				} else {
					where.chanpinmingcheng = {
						[Op.eq]: chanpinmingcheng
					}
				}
			}
			let chanpinfenlei = req.query.chanpinfenlei
			if (chanpinfenlei) {

				if (chanpinfenlei.indexOf('%') != -1) {
					where.chanpinfenlei = {
						[Op.like]: chanpinfenlei
					}
				} else {
					where.chanpinfenlei = {
						[Op.eq]: chanpinfenlei
			...
			let chanpinxiangqing = req.query.chanpinxiangqing
			if (chanpinxiangqing) {

				if (chanpinxiangqing.indexOf('%') != -1) {
					where.chanpinxiangqing = {
						[Op.like]: chanpinxiangqing
					}
				} else {
					where.chanpinxiangqing = {
						[Op.eq]: chanpinxiangqing
					}
				}
			}
            ...
            if (jwt.decode(req.headers.token).role != '管理员' && NongchanpinxinxiModel.rawAttributes.hasOwnProperty('userid')) {
				where.userid = {
	...
	api.post('/delete', async (req, res) => {
		...
		try {
			...
			await NongchanpinxinxiModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await NongchanpinxinxiModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM nongchanpinxinxi WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM nongchanpinxinxi WHERE " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM nongchanpinxinxi WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM nongchanpinxinxi WHERE " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM nongchanpinxinxi WHERE " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM nongchanpinxinxi WHERE " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "nongchanpinxinxi"
			let where = " WHERE 1 = 1 "
			let tableName = req.session.userinfo === undefined ? jwt.decode(req.headers.token).tableName : req.session.userinfo.tableName
			if(tableName == 'nongmin') {
				where += " AND nongminzhanghao = '" + jwt.decode(req.headers.token).username + "' ";
			}
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM nongchanpinxinxi " + where + " GROUP BY " + columnName + " LIMIT 10" 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			if ("nongchanpinxinxi" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM nongchanpinxinxi " + where + " GROUP BY " + xColumnName + " DESC LIMIT 10"
