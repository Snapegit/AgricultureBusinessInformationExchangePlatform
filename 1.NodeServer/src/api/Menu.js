...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
            ...
            if (jwt.decode(req.headers.token).role != '管理员' && MenuModel.rawAttributes.hasOwnProperty('userid')) {
				where.userid = {
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM menu " + where + " GROUP BY " + xColumnName + " DESC LIMIT 10"
