...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussnongchanpinxinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussnongchanpinxinxiCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussnongchanpinxinxi', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussnongchanpinxinxi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussnongchanpinxinxi');
			...
			async init(){
				...
				let res = await this.$api.info('discussnongchanpinxinxi', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussnongchanpinxinxi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussnongchanpinxinxi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
